<!DOCTYPE html>
<!-- 
Filename: updateResourceReviews.php, Kyle Dunne, CIS355, 2015-04-21
This file updates a review for a resource
...
error_reporting(E_ALL);
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            //Retrieve resource tuid
            $sql = $pdo->prepare("SELECT resourceTUID from iResourceReviews where tuid = ".$id);
            $sql->execute();
            $row = $sql->fetch();
            $resource = $row[0];
            $sql = "UPDATE iResourceReviews set title = ?, score = ?, text = ? WHERE tuid = ?";
            $q = $pdo->prepare($sql);
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT title, score, text, resourceTUID FROM iResourceReviews where tuid = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <?php
                    echo '<form class="form-horizontal" action="updateResourceReviews.php?id='.$id.'" method="post">';
                    ?>
                    <!--Title input-->
