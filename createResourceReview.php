<!DOCTYPE html>
<!-- 
Filename: createResourceReviews.php, Kyle Dunne, CIS355, 2015-04-21
This file allows a user to add a review of a resource
   ...
   else
   {
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            $pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "INSERT INTO iResourceReviews (resourceTUID, score, title, text) values(?, ?, ?, ?)";
            $q = $pdo->prepare($sql);
            $q->execute(array($resource, $score, $title, $text));
            Database::disconnect();
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    <div class="row">
                        <?php echo '<h3>Reviewing '.$resourceTitle.'</h3>';?>
                    </div>
                    <?php
                    echo '<form class="form-horizontal" action="createResourceReview.php?id='.$resource.'" method="post">';
                    ?>
                      <!--Title input-->
